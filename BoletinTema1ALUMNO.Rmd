...
author: "David Camarena Sanchez (dacasan5)"
...
```{r}
commonAnimals <- intersect(mamiferos, animales)
commonAnimals
...
setdiff(rownames(mamiferos), rownames(animales))
setdiff(rownames(animales), rownames(mamiferos))

# Respecto al tipo de animales en la variable animales hay dinosaurios, los cuales son oviparos, por lo que no entran en la categoria de mamiferos. Otros simplemente no estan escritos de la misma forma.
```

e.  Determina las diferencia entre los animales que no aparecen en ambos conjuntos.

```{r}
# Hay algunos que no estan y otros que no estan escritos de la misma forma. 
...
# Se podría decir que si siguen una distribucion Gaussiana, ya que no se salen del rango.
```
...
3.  La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    lista_librerias <- installed.packages()
    nrow(lista_librerias)
    ```
...
# Ambas gráficas verifican la condición, por lo que siguen una distribución Gaussiana.
```
...
#Diagrama de dispersión global (Apartado a).
plot(cabbages$VitC, cabbages$Cult)
```
